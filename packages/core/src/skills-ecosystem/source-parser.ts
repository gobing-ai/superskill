/**
 * Parse a skill source string (GitHub/GitLab/shorthand/git/local/well-known)
 * into a structured {@link ParsedSource}.
 *
 * Ported verbatim (behavior-identical) from vercel-labs/skills
 * (vendors/skills/src/source-parser.ts); `isSubpathSafe` is ported from
 * vendors/skills/src/skills.ts.
 ...
 * Supports: local paths, GitHub URLs, GitLab URLs, GitHub shorthand, well-known
 * URLs, and direct git URLs.
 */
export function parseSource(input: string): ParsedSource {
    ...
    if (gitlabPrefixMatch) {
        return parseSource(
            appendFragmentRef(`https://gitlab.com/${gitlabPrefixMatch[1] ?? ''}`, fragmentRef, fragmentSkillFilter),
        );
    }
