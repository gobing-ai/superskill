/**
 * Parity tests for path traversal prevention in subpath handling.
 *
 * Fixtures ported verbatim from vercel-labs/skills
 * (vendors/skills/tests/subpath-traversal.test.ts) — same inputs, same
 * expected outputs; only the test-runner import (vitest → bun:test) and
 * module paths changed (`isSubpathSafe` lives in our source-parser module
 * instead of the vendor's skills.ts).
 *
...
import { isSubpathSafe, parseSource, sanitizeSubpath } from '../../src/skills-ecosystem/source-parser';
...
describe('parseSource rejects traversal in subpaths', () => {
    describe('GitHub tree URLs with path traversal', () => {
        it('rejects .. in GitHub tree URL subpath', () => {
            expect(() => parseSource('https://github.com/owner/repo/tree/main/../../etc')).toThrow('Unsafe subpath');
        });
    ...
    describe('GitLab tree URLs with path traversal', () => {
        it('rejects .. in GitLab tree URL subpath', () => {
            expect(() => parseSource('https://gitlab.com/owner/repo/-/tree/main/../../etc')).toThrow('Unsafe subpath');
        });
