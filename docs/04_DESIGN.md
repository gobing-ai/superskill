...
| `stale: …` | Version or hash differs; one row per plugin (current upstream, no history). Equal installed/upstream versions are content drift: `content changed, version <v> unchanged (…)`; a bump keeps `<old> → <new>`. Text caps the changed-path list at 5 entries + `+N more` (the row and `--json` always carry every path); a partially stale merged row appends `[stale on: <targets>]` (task 0136 R8/R11/R12). `--check` text output ends with `Summary: <s> stale, <c> up to date[, <u> not checked][, <l> legacy][, <x> unavailable]` and `Run: superskill update` only when something is stale (task 0136 R10) | `--check` → 1 |
