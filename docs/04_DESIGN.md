...
| `--check` | Report only; never writes manifests or dest files. Exit 1 if any stale row and no unavailable row; `--help` states the 0/1/2 exit contract (0 nothing stale, 1 stale under `--check` or an apply failure, 2 an unavailable upstream) (task 0136 R17) |
...
| --- | --- | --- |
| `up to date` | Version and (marketplace) upstream hash match; a disagreeing `marketplace.json`/`plugin.json` declaration appends `(note: marketplace.json declares <a>, plugin.json declares <b>)` — the compare keeps marketplace-first precedence (task 0136 R9) | 0 if all current/legacy |
| `stale: …` | Version or hash differs; one row per plugin (current upstream, no history). Equal installed/upstream versions are content drift: `content changed, version <v> unchanged (…)`; a bump keeps `<old> → <new>`. Text caps the changed-path list at 5 entries + `+N more` rendered as indented continuation lines under the row head (with `stale on: <targets>` and `note: …` following; the `--json` always carries every path); single-line rows (no changed paths) keep inline `[stale on: <targets>]` (task 0136 R8/R11/R12). `--check` text output ends with `Summary: <s> stale, <c> up to date[, <u> not checked][, <l> legacy][, <x> unavailable]` and `Run: superskill update` only when something is stale (task 0136 R10) | `--check` → 1 |
| `installed before manifest support - run \`superskill install <name>\` to adopt` | Known candidate, missing/corrupt/unsupported manifest; the row names the adopt command (task 0136 R15) | 0 (guidance) |
| `upstream unavailable (<locator>)`, `(<locator>): <reason>` when the row carries one; locator-less rows drop the empty parens | Locator or npm lookup failed with a specific reason (missing locator directory → `locator path missing`, manifest/parse, network/registry); other rows still print (task 0136 R21) | 2 (wins over 1) |
...
Bare `update` re-runs `executeInstall` for stale **marketplace** plugins (0123 refreshes the manifest), prints `To upgrade superskill and its bundled plugins, run: npm i -g @gobing-ai/superskill@latest` once for stale **bundled** plugins, and then applies stale skills via `updateSkills(precheck)` built from this run's `checkSkills` rows (the apply set is this run's stale rows only; `updateSkills` re-hashes each applied row in its no-op pre-check before reinstalling rather than reusing the check hash; a failed skill reinstall exits 1) (task 0135 R3). Each applied item prints `Updating <name>…` before its reinstall and a completed run ends with `Updated <n> of <m>.` counting successes over attempted items (stale plugins + stale skills); a failed skill reinstall is named on its own line without blocking the rest, and a thrown plugin reinstall failure still aborts the run with exit 1 (task 0137 R13). `--check` is read-only, and `--check --json` stdout stays the single envelope — no progress, count, or remedy lines (task 0137 R16).
...
| `skill update [names...]` | `-g, --global`, `-y, --yes`, `--json`, `--check` (read-only; exit 0 current/unchecked, 1 stale, 2 unavailable) |
