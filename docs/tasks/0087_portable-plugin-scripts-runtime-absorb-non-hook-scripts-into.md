...
updated_at: "2026-09-05T04:00:57.949Z"
---
...
- `plugins/cc/skills/anti-hallucination/SKILL.md:217` — "Platforms Without Hooks" direct-validation item re-labeled: staged-path command first as **standard**, `script run` demoted to **optional** (registry form). The Phase 4 pending workflow (item 1) and the `script path` standard + `script run` optional summary in the task-source revision (`git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/SKILL.md`, original lines 221-238) were already correct and remained unchanged during that task.
- `plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:11-33` — guide intro inverted: staged path introduced as **standard** (install-time staging + `script path` resolution, citing ADR-023), registry as **Optional (registry)**. Removed the registry-boosting claims ("recommended recipe", "not the preferred way to reach this engine") and the staged-twin demotion ("only useful if you specifically need an FS entrypoint"). Both adapters still share the same engine, exit codes, and input modes.
- `plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:62-89` — Host-Side Validation and Pipe-Final-Output example pairs reordered: staged-path command is the "Standard form", `script run` the "Optional form"; identical `RESPONSE_TEXT`/stdin semantics preserved.
- The task-source revision (`git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md`, original lines 89-111) records the Cross-Agent Enforcement "Until Phase 4 lands" guidance: staged path first, `script run` optional; the workflow remained pending (blocked on Spur `agent.run` output capture, ADR-015) per R3.
- `plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:97-107` and `:109-126` — Reviewer Workflow and Structured Output validation commands reordered staged-path-first with the registry as the optional alternative.
- `plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:128-135` — Design Rule now reads **standard**: staged path; **optional**: `script run`, mirroring ADR-023's vocabulary.
- `plugins/cc/skills/anti-hallucination/references/guard-implementation.md:68-72` — no-hook-platforms paragraph now presents the staged `.mjs` twin as the standard form and `superskill script run cc validate-response` as the optional registry form; the hook configuration section (portable `superskill hook run` command, `minCliVersion` floor) is untouched.
...
| R2 | MET | `plugins/cc/skills/anti-hallucination/SKILL.md:217` (standard staged path, optional registry); `plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:11-31` (intro order), `:64-89` (usage pairs), `:128-135` (Design Rule); `plugins/cc/skills/anti-hallucination/references/guard-implementation.md:68-72`; `plugins/cc/README.md:146` (validator row). Matches ADR-023 dual contract. |
| R3 | MET | `rg` residual for preferred/recommended-recipe/secondary registry claims → none in the four consumer docs. Phase 4 remained pending in the historical task-source revisions (`git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md`, original lines 89-107; `git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/SKILL.md`, original line 221). The current docs state that no cross-agent workflow YAML ships. Dev-repo bun warning (`plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:35-38`) is a negative contrast, not a primary recipe. |
...
| R2 | MET | `plugins/cc/skills/anti-hallucination/SKILL.md:217` (direct-validation item: **standard** staged path first, **optional** registry); `references/non-hook-enforcement.md:11-33` (intro inversion), `:62-89` (host-side/pipe example pairs), `git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md` (historical lines 89-111; cross-agent guidance), `:122-135` (reviewer, structured output, Design Rule); `references/guard-implementation.md:68-72`; `plugins/cc/README.md:146` (validator row re-labeled; hook row `:135` unchanged). Ordering matches ADR-023 and `docs/04_DESIGN.md`. |
| R3 | MET | Registry-boosting claims removed in `non-hook-enforcement.md` diff ("recommended recipe", "not the preferred way to reach this engine", "only useful if you specifically need an FS entrypoint"); staged-twin demotion gone. Phase 4 workflow was still marked pending in the historical task-source revisions (`git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md`, original lines 89-108; `git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/SKILL.md`, original line 221). Current docs state that no cross-agent workflow YAML ships. Grep for residual primary/preferred/recommended/secondary claims in the four consumer docs finds only unrelated usages in the historical task-source revision (`git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/SKILL.md`, original line 113; `git show fc87823f32d540ef4fcd7785433e27b362b9f7c9:plugins/cc/skills/anti-hallucination/references/anti-hallucination-research.md`, original line 186) and the current dev-repo warning (`plugins/cc/skills/anti-hallucination/references/non-hook-enforcement.md:35-38`), which cautions against bun-of-source, not the registry. |
