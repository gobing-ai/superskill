...
updated_at: "2026-08-01T02:22:33.430Z"
feature_id: F3
---
...
### Design
Register `install` in `cli.ts` and **remove the scaffold `add` demo command**. Flow: resolve plugin → `mapPluginToRulesync` (G12) → ConversionPipeline (G13) → `runRulesync` with `outputRoots` set (G13/ADR-010) → dispatch only the targets rulesync can't write.
...
| # | Title | Dimension | Location | Recommendation |
|---|-------|-----------|----------|----------------|
| 1 | FIXED: Install flow skipped the G13 conversion pipeline | Correctness | `apps/cli/src/commands/install.ts:121` | Added target-specific `.rulesync/.targets/<target>` input roots and applied frontmatter normalization, slash-command translation, colon rewrite, and Pi subagent conversion before rulesync. |
