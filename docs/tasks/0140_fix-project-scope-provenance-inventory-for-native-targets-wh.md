...
updated_at: "2026-09-14T16:58:18.163Z"
...
|-------------|--------|----------|
| R1 | MET | Home fallback writes through the unchanged project writer: `apps/cli/src/commands/install.ts:2297-2313` (`snapshotFiles(realHomeRoot, inHome)` at `:2311`, `args.writer(args.outputRoot, …)` at `:2330`), non-emptiness enforced by the frozen throw at `:2306-2310` — all re-read this run. Test `apps/cli/tests/commands/install-manifest.test.ts:479-518` (home-only claude cache → readable project manifest, home-relative keys) re-read and re-executed this run (36 pass / 0 fail). |
| R2 | MET | Field + TSDoc `packages/core/src/operations/install-manifest.ts:56-59`; reader rejects any non-`'home'` value with `Install manifest installedRoot must be home: <label>` and preserves the field (`:284-290`, spread at `:304`) — re-read this run. CLI sets `installedRoot = 'home'` only in the home branch (`apps/cli/src/commands/install.ts:2312`) and spreads it only when set (`:2328`). Tests: `packages/core/tests/operations/install-manifest.test.ts:203-224` (round-trip + rejections) re-read and re-executed this run. |
| R3 | MET | lstat-before-realpath symlink skip and realpath dedupe `apps/cli/src/commands/install.ts:2280-2283`; scope root realpath'd with `existsSync` fallback `:2290-2291`; home realpath'd in the fallback `:2304`; both sides canonical before `relative()` via `isUnderRoot` `:2240-2243` — all re-read this run. Test `:520-572` (symlinked `HOME_DIR` + realpath-distinct grok-reported path → home-rooted manifest) re-read and re-executed this run. |
| R4 | MET | Scope-root-first precedence `:2295-2296`; home branch gated to `claude`/`omp`/`grok` at `:2303`; both-empty → the unchanged frozen error `:2306-2310`; writer reached only after the branch `:2330` — all re-read this run. Guard tests `:619-655` (non-native scope-rooted unaffected), `:657-683` (non-native home inventory refused), `:685-708` (frozen error when the scope root does not exist) re-read and re-executed this run. |
| R5 | MET | Global scope resolves `outputRoot` to home (`apps/cli/src/commands/install.ts:413`, re-read this run), so home-rooted receipts are in-scope and the field is never emitted (`:2328` spread only when set). Both install-manifest suites pass unedited this run: 36 pass / 0 fail (the tree was clean at re-audit start; this pass touched only `fetch.ts` comments). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: Project-scope provenance inventory resolves native-target content under $HOME | MET | test | `apps/cli/tests/commands/install-manifest.test.ts:479-518` (project-scope claude with content under `$HOME` → `installedRoot === 'home'`, SHA-256-matched home-relative key) and `:520-572` (symlinked `HOME_DIR`, realpath-distinct host-reported path, target grok) — re-read and re-executed this run: 36 pass / 0 fail across both suites. Live isolated-HOME E2E (claude/grok manifests `installedRoot: "home"`, 370 files) is recorded original-run command evidence, not re-executed in this re-audit. |
| AC2 (claude home-only, automated) | MET | test | `:479-518`, passed this run. |
| AC3 (symlinked HOME_DIR grok, automated) | MET | test | `:520-572`, passed this run. |
| AC4 (both-empty throws, automated) | MET | test | `:685-708` (frozen error, no manifest, isolated `HOME_DIR`), passed this run. |
| AC5 (core field round-trip, automated) | MET | test | `packages/core/tests/operations/install-manifest.test.ts:203-224`, passed this run. |
| AC6 (no regression) | MET | command | This run: `bun run lint` → biome 235 files clean + both workspace typechecks exit 0; both suites 36 pass / 0 fail unedited, no `.skip`/`.todo`/`.only`. |
