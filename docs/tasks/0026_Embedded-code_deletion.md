...
updated_at: "2026-08-01T02:24:06.886Z"
feature_id: H6
---
...
### Background
Remove the embedded code from all five skills, now that H4+H5 no longer reference it. Capability lives in the superskill CLI (Phases 1-2); the bundled scripts/templates/tests are dead weight and violate the 'no embedded execution' invariant (design invariant #3). cc-hooks's bash machinery (emitters/, schema/) has no CLI home in Phase 3 (rulesync-backed hooks are Phase 5). STRICT ordering invariant (design §5, invariant #4): delete ONLY after H4+H5 stop referencing the deleted paths. Plugin-only — apps/cli/'s own tests/templates/scripts (builder.ts, bump.ts) are unrelated and must NOT be deleted. vendors/ untouched (ADR-004). Design: design-doc-phase3.md §5. Owning feature: H6.
### Requirements
- [x] **R1** — **Pre-deletion gate held** (design §5 ordering invariant #4): before any delete, both `rg "scripts/" plugins/cc/` and `rg "bun .*\.ts" plugins/cc/` are **empty**. If either has hits → STOP and fix the referencing file in H4/H5 first.
...
**Out of scope:** any SKILL.md/agent/command edit (those land in H4/H5; if the gate fails, fix there).
### Q&A



### Design
**Scope:** Delete embedded code from all five `plugins/cc/skills/<skill>/` dirs after H4 (0024) + H5 (0025) stopped referencing deleted paths. Plugin-only — `apps/cli/` and top-level `scripts/` untouched.
...
**Pre-deletion gate (R1, mandatory):** `rg "scripts/" plugins/cc/` and `rg "bun .*\.ts" plugins/cc/` must both be empty. H4 (0024) rewrote SKILL.md files to use `superskill <type> <op>`. H5 (0025) rewrote slash commands. Both verified clean in their task closures. Gate confirms no live references to deleted paths remain.

**Ordering invariant (design §5, #4):** Delete ONLY after gate passes. The gate is the safety check — if it fails, fix the referencing file in H4/H5 scope first, do not delete.
...
- **Regression risk:** Low. The deleted code was already superseded by the `superskill` CLI (H4/0024 rewrote SKILL.md files; H5/0025 rewrote slash commands). The gate-clearing reference fixes are surgical documentation path replacements, no structural changes.

**Out-of-Scope Compliance**

- SKILL.md structural rewrites → done in 0024 (H4), not touched here beyond reference-path cleanup already required by the deletion gate
- Slash command rewrites → done in 0025 (H5), not touched
- Namespace migration → done in 0023 (H3), not touched
...
- Feature: [H6](../features/H6_embedded-code-deletion.md)
