...
updated_at: "2026-09-04T23:47:39.965Z"
priority: P0
feature_id: G
...
### Solution
Implemented and force-reverified R1–R10 across `packages/core`; public signatures and workspace boundaries remain unchanged.

- **R1 path safety:** `pathsNestOrEqual` is component-aware, resolves existing symlink ancestors, and fails closed across filesystem roots (`packages/core/src/content/paths.ts:60`). Recursive-clean callers retain their pre-delete refusal. Residual fixtures cover `..plugin` children and symlinked ancestors (`packages/core/tests/content/paths.test.ts:57`, `packages/core/tests/content/paths.test.ts:67`).
- **R2 marketplace containment:** `resolvePlugin` realpaths and checks the plugin root before enumerating it (`packages/core/src/marketplace.ts:184`, `packages/core/src/marketplace.ts:191`, `packages/core/src/marketplace.ts:198`). Tests cover escaped source/pluginRoot symlinks and an in-root symlink (`packages/core/tests/marketplace.test.ts:280`, `packages/core/tests/marketplace.test.ts:292`, `packages/core/tests/marketplace.test.ts:302`).
- **R3 serialized mutation:** the per-lock atomic-directory guard uses a bounded wait, dead-PID recovery, and `finally` release (`packages/core/src/skills-ecosystem/locks.ts:336`); add/remove take it before the authoritative read and through transaction completion (`packages/core/src/skills-ecosystem/operations.ts:348`, `packages/core/src/skills-ecosystem/operations.ts:496`). Serialization and union fixtures are at `packages/core/tests/skills-ecosystem/locks.test.ts:421` and `packages/core/tests/skills-ecosystem/locks.test.ts:464`.
- **R4 lock preservation:** only `ENOENT` means absence (`packages/core/src/skills-ecosystem/locks.ts:217`); readers surface other I/O/shape failures, and raw writers validate the complete on-disk top-level/version/skills shape before replacement (`packages/core/src/skills-ecosystem/locks.ts:230`, `packages/core/src/skills-ecosystem/locks.ts:423`). Tests prove malformed JSON, `ENOTDIR`, and invalid-skills bytes are not synthesized or overwritten (`packages/core/tests/skills-ecosystem/locks.test.ts:579`, `packages/core/tests/skills-ecosystem/locks.test.ts:640`, `packages/core/tests/skills-ecosystem/locks.test.ts:689`).
- **R5 hostile dictionary keys:** fresh and parsed skill maps use null-prototype records (`packages/core/src/skills-ecosystem/locks.ts:385`, `packages/core/src/skills-ecosystem/locks.ts:406`); `__proto__` round-trips as an own key (`packages/core/tests/skills-ecosystem/locks.test.ts:725`).
- **R6 YAML scalar safety:** mapper, frontmatter walker, command/subagent fallbacks, and Pi output quote final scalar values (`packages/core/src/mapper.ts:95`, `packages/core/src/pipeline/frontmatter-walk.ts:68`, `packages/core/src/pipeline/adapt-command.ts:30`, `packages/core/src/pipeline/adapt-subagent.ts:102`). Pi's multi-skill field is one quoted CSV scalar (`packages/core/src/pipeline/adapt-subagent.ts:107`). Hostile names parse back exactly with no injected sibling key (`packages/core/tests/mapper.test.ts:470`, `packages/core/tests/pipeline/adapt-command.test.ts:60`, `packages/core/tests/pipeline/adapt-subagent.test.ts:56`, `packages/core/tests/pipeline/frontmatter-walk.test.ts:208`).
- **R7 fence-aware migration:** `dedupeLines` preserves fenced content, matching-character/width closure, and trailing blanks in an unclosed fence while retaining outside dedupe (`packages/core/src/operations/migrate.ts:99`, `packages/core/tests/operations/migrate.test.ts:117`).
- **R8 flat packaging:** `resolveSkillEntry` distinguishes directory and flat forms; flat input derives the file-stem name and copies only that file to required `SKILL.md` (`packages/core/src/operations/package.ts:38`, `packages/core/src/operations/package.ts:90`, `packages/core/src/operations/package.ts:134`). The sibling-isolation fixture is `packages/core/tests/operations/package.test.ts:151`.
- **R9 bounded acquisition:** fixed ceilings are 8 concurrent requests, 256 candidates, 2,048 files, and 16/2/32 MiB payloads; one local mapper drives all three fan-outs and limit errors remain terminal (`packages/core/src/skills-ecosystem/fetch.ts:90`, `packages/core/src/skills-ecosystem/fetch.ts:98`, `packages/core/src/skills-ecosystem/fetch.ts:112`). Exact candidate/file boundaries and over-limit pre-work rejection are covered at `packages/core/tests/skills-ecosystem/fetch.test.ts:705`, `packages/core/tests/skills-ecosystem/fetch.test.ts:722`, `packages/core/tests/skills-ecosystem/fetch.test.ts:761`, and `packages/core/tests/skills-ecosystem/fetch.test.ts:785`.
- **R10 Markdown destinations:** extraction handles angle/bare-title forms, strips raw URI suffixes before guarded percent decoding, and preserves encoded `#`/`?` as filename bytes (`packages/core/src/operations/validate.ts:106`, `packages/core/src/operations/validate.ts:150`, `packages/core/src/operations/validate.ts:191`). Tests cover titles, percent encoding, encoded separators, query/fragment stripping, and invalid encoding (`packages/core/tests/operations/validate.test.ts:598`, `packages/core/tests/operations/validate.test.ts:615`, `packages/core/tests/operations/validate.test.ts:632`, `packages/core/tests/operations/validate.test.ts:649`, `packages/core/tests/operations/validate.test.ts:666`).

Forced re-verification repaired seven residuals found in the prior PASS state: cross-root overlap returned false; marketplace enumeration preceded realpath containment; `ENOTDIR` was treated as absence and bypassed serialization; raw writers accepted invalid `skills`; Pi emitted invalid multi-skill YAML; unclosed-fence trailing blanks were trimmed; and encoded query/fragment filename characters were decoded before suffix stripping. The smallest shared guards/parsers were corrected and residual-proof tests added.

Verification: focused R1–R10 suites 314/314 pass; lock/operation regression 56/56 pass; `bun run lint`, `bun run test` (2,185 pass, 0 fail; 98.88% lines, 99.57% functions), `bun run build`, and `bun run spur-check` all exit 0. `git diff --check` passes; the worktree contains only the intentional task-0126 source, test, and harness-managed evidence changes.
### Testing
...
|-------------|--------|----------|
| R1 | MET | Component/symlink-aware overlap and cross-root fail-closed behavior are implemented at `packages/core/src/content/paths.ts:60`; residual tests cover `..plugin` and symlinked ancestors at `packages/core/tests/content/paths.test.ts:57` and `packages/core/tests/content/paths.test.ts:67`. |
| R2 | MET | `resolvePlugin` realpaths and enforces containment before `readdirSync` at `packages/core/src/marketplace.ts:184`, `packages/core/src/marketplace.ts:191`, and `packages/core/src/marketplace.ts:198`; escaped and in-root symlink tests are at `packages/core/tests/marketplace.test.ts:280`, `packages/core/tests/marketplace.test.ts:292`, and `packages/core/tests/marketplace.test.ts:302`. |
| R3 | MET | The bounded per-lock atomic-directory guard is at `packages/core/src/skills-ecosystem/locks.ts:336`; add/remove acquire it before authoritative reads at `packages/core/src/skills-ecosystem/operations.ts:348` and `packages/core/src/skills-ecosystem/operations.ts:496`. Serialization/union tests are at `packages/core/tests/skills-ecosystem/locks.test.ts:421` and `packages/core/tests/skills-ecosystem/locks.test.ts:464`. |
| R4 | MET | Only `ENOENT` is absence at `packages/core/src/skills-ecosystem/locks.ts:217`; raw writers validate on-disk top-level/version/skills shapes at `packages/core/src/skills-ecosystem/locks.ts:230`. Malformed JSON, `ENOTDIR`, and invalid-skills preservation tests are at `packages/core/tests/skills-ecosystem/locks.test.ts:579`, `packages/core/tests/skills-ecosystem/locks.test.ts:640`, and `packages/core/tests/skills-ecosystem/locks.test.ts:689`. |
| R5 | MET | Fresh and parsed skill maps use null-prototype records at `packages/core/src/skills-ecosystem/locks.ts:385` and `packages/core/src/skills-ecosystem/locks.ts:406`; `__proto__` own-key JSON round-trip is tested at `packages/core/tests/skills-ecosystem/locks.test.ts:725`. |
| R6 | MET | Every generated name path uses quoted YAML scalars: mapper `packages/core/src/mapper.ts:95`, walker `packages/core/src/pipeline/frontmatter-walk.ts:68`, command fallback `packages/core/src/pipeline/adapt-command.ts:30`, subagent/Pi output `packages/core/src/pipeline/adapt-subagent.ts:102`, and Pi multi-skill scalar `packages/core/src/pipeline/adapt-subagent.ts:107`. Hostile parse-back tests cover all paths at `packages/core/tests/mapper.test.ts:470`, `packages/core/tests/pipeline/adapt-command.test.ts:60`, `packages/core/tests/pipeline/adapt-subagent.test.ts:56`, and `packages/core/tests/pipeline/frontmatter-walk.test.ts:208`. |
| R7 | MET | Fence-aware preservation and matching closer semantics are implemented at `packages/core/src/operations/migrate.ts:99`; the residual unclosed-fence trailing-blank test is at `packages/core/tests/operations/migrate.test.ts:117`. |
| R8 | MET | Flat/directory entry resolution and required primary copy are at `packages/core/src/operations/package.ts:38`, `packages/core/src/operations/package.ts:90`, and `packages/core/src/operations/package.ts:134`; flat sibling isolation is tested at `packages/core/tests/operations/package.test.ts:151`. |
| R9 | MET | Acquisition errors and fixed ceilings are at `packages/core/src/skills-ecosystem/fetch.ts:90` and `packages/core/src/skills-ecosystem/fetch.ts:98`; the shared bounded mapper is at `packages/core/src/skills-ecosystem/fetch.ts:112`. Exact/over candidate and materialization limits are tested at `packages/core/tests/skills-ecosystem/fetch.test.ts:705`, `packages/core/tests/skills-ecosystem/fetch.test.ts:722`, `packages/core/tests/skills-ecosystem/fetch.test.ts:761`, and `packages/core/tests/skills-ecosystem/fetch.test.ts:785`. |
| R10 | MET | Link extraction and suffix-before-decode normalization are at `packages/core/src/operations/validate.ts:106`, `packages/core/src/operations/validate.ts:150`, and `packages/core/src/operations/validate.ts:191`; optional-title, percent-encoded, encoded-separator, query/fragment, and invalid-encoding fixtures are at `packages/core/tests/operations/validate.test.ts:598`, `packages/core/tests/operations/validate.test.ts:615`, `packages/core/tests/operations/validate.test.ts:632`, `packages/core/tests/operations/validate.test.ts:649`, and `packages/core/tests/operations/validate.test.ts:666`. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Domain operations in packages/core run without the CLI. | MET | test | Direct core API fixtures for R1–R10 pass: 314/314 focused tests and 2,185/2,185 repository tests. Implementations remain under `packages/core/src`; tests invoke exported core functions without Commander or CLI argv/stdout/exit coupling. |
| Cross-workspace imports use workspace aliases. | MET | command | `git diff --name-only` shows only `packages/core`, its adjacent tests, and harness-managed task evidence; `rg` finds no new cross-workspace relative import. `bun run lint`, `bun run build`, and `bun run spur-check` exit 0, confirming no workspace/dependency/config/CLI drift. |
