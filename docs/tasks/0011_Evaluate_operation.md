...
updated_at: "2026-08-01T02:23:21.629Z"
feature_id: G24
---
...
Evaluate dispatches to type-specific evaluator functions from G22 (`quality/skill.ts`, `quality/command.ts`, `quality/agent.ts`, `quality/hook.ts`, `quality/magent.ts`). Each evaluator takes the raw file content string and a target agent identifier, returning a `QualityReport`. The operation supports `--json` for machine-readable output, `--save` to persist results to the SQLite evaluation store (F4), and `--target` for agent-specific evaluation context.

Unlike validate, evaluate is **never called with exit code ≠ 0 for low scores** — a score of 0.0 is a valid evaluation result. Only file-not-found conditions produce exit code 2 (mapped by F5).
### Requirements
...
**R2** — `EvaluateOptions` type: `{ target?: Target, json?: boolean, save?: boolean, operation?: string, adapter?: DbAdapter }`. `target` defaults to `'claude'` (ADR-013 — `target_agent` is never null). `json` selects output format. `save` triggers persistence. `operation` allows G25 (refine) to override the operation string stored in the evaluations table (defaults to `'evaluate'`; refine passes `'refine'`). `adapter` lets a caller inject an already-open store (G26 evolve passes its in-memory `DbAdapter` so the verify-step `--save` writes to the same DB; tests pass a `:memory:` adapter). When `adapter` is absent and `save` is set, `evaluate` opens its own store via `await openStore()`.

**R3** — `EvaluationResult` type alias for `QualityReport` (from `quality/dimensions.ts`, G22): `{ content: string, type: ContentType, target: Target, aggregate: number, dimensions: Record<string, DimensionScore> }`.

**R4** — `DimensionScore` type (from G22): `{ score: number, note: string }`. `score` is 0.0–1.0. `note` is a one-line explanation of the score.
...
**R9** — **`--save` persistence**: opens the SQLite store via `await openStore()` from F4's `store/db.ts`, then calls `await new EvaluationDao(adapter).insertEvaluation({ ... })`. The row values:
- `content_type`: the `type` parameter
- `content_name`: `resolveContentName(resolvedPath)` from G21's `content/identity.ts` (the canonical content name — directory-stripped, extension-stripped; ADR-013)
...
- `file_hash`: `hashContent(resolvedPath)` from G21's `content/hash.ts` (SHA-256 hex of file bytes)
...
**R10** — **file_hash computation**: use `hashContent(filePath)` from `content/hash.ts` (G21). Implementation uses `Bun.CryptoHasher` with SHA-256 (or `node:crypto.createHash('sha256')` as fallback). The hash is computed on the raw file bytes at evaluation time and stored with `--save`. This enables evolve (G26) to detect content changes between evaluations.

**R11** — **File path resolution**: same logic as validate — `resolveContentPath(type, nameOrPath, opts?)` from `content/identity.ts` (G21). Bare name → `cwd/<name>.md`. Explicit path → as-is. If the resolved path does not exist, return early with an error (the F5 command layer maps this to exit 2).
...
**R13** — **Content type coverage**: works for all 5 content types via dispatch to the appropriate G22 evaluator. Each evaluator handles its type's dimension set independently.

**R14** — **Exit code behavior** (mapped by F5): 0 on successful evaluation (even if scores are low — evaluation failure ≠ quality failure). 2 only for file not found or unreadable.

**R15** — **Output formatter**: export `formatEvaluationReport(report: QualityReport, json?: boolean): string`. Separated so the CLI layer (F5) and refine (G25) can reuse formatting without re-implementing.
### Q&A
...
**Imports**:
- `ContentType`, `QualityReport`, `DimensionScore` from `quality/dimensions.ts` (G22)
- `evaluateSkill` from `quality/skill.ts` (G22)
- `evaluateCommand` from `quality/command.ts` (G22)
- `evaluateAgent` from `quality/agent.ts` (G22)
- `evaluateHook` from `quality/hook.ts` (G22)
- `evaluateMagent` from `quality/magent.ts` (G22)
- `resolveContentPath`, `resolveContentName` from `content/identity.ts` (G21)
- `hashContent` from `content/hash.ts` (G21)
- `openStore`, `EvaluationDao` from `store/` (F4) — only imported when `--save` is set; dynamic import acceptable if store is not always needed
...
1. Resolve path: `const resolvedPath = resolveContentPath(type, nameOrPath, opts)`. If file does not exist, throw a typed error or return early — caller (F5) maps to exit 2.
...
6. If `opts?.save`:
   ...
   - Reuse or open store: `const store = opts.adapter ?? await openStore()` (inject from G26 evolve / tests; otherwise open per F4)
   - Create DAO: `const dao = new EvaluationDao(store)` (1-arg constructor — the DAO hardcodes its table/PK/name; see F4)
   - Insert: `await dao.insertEvaluation({ content_type: type, content_name: resolveContentName(resolvedPath), target_agent: resolvedTarget, operation: opts.operation ?? 'evaluate', aggregate: report.aggregate, dimensions: report.dimensions, file_hash: fileHash })` — pass `dimensions` as the object; the DAO `JSON.stringify`s it on write (F4 R7). Do **not** double-stringify here.
...
- Dispatches to G22 type-specific evaluators via a simple `Record<ContentType, EvaluatorFn>` map
- File path resolution via G21 `content/identity.ts`
- File hashing via G21 `content/hash.ts`
- Store persistence via F4 `EvaluationDao` when `--save` is set
...
7. Implement `file_hash` computation via `hashContent` from G21
...
- Phase 7: 0×P1, 0×P2, 0×P3, 1×P4 (cosmetic, see below). Read-only evaluator + local SQLite write via parameterized F4 DAO — no injection/secrets/auth surface; lazy store import on `--save`; `--save` errors → stderr, never fail the evaluation.
- Phase 8: 15/15 requirements MET, no scope drift, no untraced code. Dependencies confirmed: 5 G22 evaluators, G21 identity/hash, F4 openStore/EvaluationDao (DAO JSON-stringifies `dimensions` internally — no double-stringify in evaluate).
...
- **R5 (Type dispatch):** `EVALUATORS` record mapping `ContentType` to G22 evaluator functions — skill, command, agent, hook, magent.
- **R6 (Aggregate):** Uses G22's `computeAggregate` via evaluators — equal-weighted mean.
...
- **R10 (file_hash):** Uses `hashContent` from G21 — SHA-256 via Bun.CryptoHasher.
- **R11 (File path):** Delegates to `resolveContentPath` (G21). Throws with `code: 2` on missing/unreadable files.
...
- **R15 (Formatter):** `formatEvaluationReport` exported separately for reuse by F5 CLI and G25 refine.
### Testing
...
### References
- `docs/features/G24_evaluate-operation.md` — feature spec
...
- `docs/features/G22_quality-dimension-definitions.md` — QualityReport, DimensionScore, evaluator functions
- `docs/features/F4_sqlite-data-store.md` — openStore, EvaluationDao
- `docs/features/G21_template-content-io-foundation-scaffold-operation.md` — resolveContentPath, resolveContentName, hashContent
