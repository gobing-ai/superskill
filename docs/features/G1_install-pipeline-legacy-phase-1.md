---
schema_version: 1
id: "G1"
name: "Install pipeline (legacy Phase 1)"
status: active
priority: P2
tags: []
created_at: "2026-08-01T02:03:55.572Z"
updated_at: "2026-09-14T14:04:14.923Z"
---

# G1: Install pipeline (legacy Phase 1)

## Goal
Phase 1 legacy group — target taxonomy, plugin→rulesync mapping, conversion pipeline, and marketplace resolution: the distribution substrate under superskill install. Children are completed legacy features converted from F001–F006.
## Scope
- In: G11–G14.
- Out: the install command surface itself (→ F3); install reporting (→ F6).
## Acceptance Criteria

```gherkin
Feature: Install pipeline (legacy Phase 1)

  Scenario: Remote marketplace materialization survives binary and oversized assets
    Given a remote --marketplace repo containing binary or oversized assets
    When install cold-materializes the repo into the marketplace cache
    Then acquisition completes without AcquisitionLimitError

  Scenario: Project-scope provenance inventory resolves native-target content under $HOME
    Given a project-scope install with a symlink-free HOME
    When the provenance inventory is built for the plugin target
    Then every installed file resolves and provenance reports non-empty
```

## Tasks

<!-- AUTO-GENERATED by spur feature refresh -->
| WBS | Task | Status |
| --- | ---- | ------ |
| 0139 | Fix remote marketplace materialization failing on binary and oversized repository assets | todo |
| 0140 | Fix project-scope provenance inventory for native targets whose plugin content lives under $HOME | testing |
<!-- END AUTO-GENERATED -->

## Notes

## History

- 2026-09-14T14:04:14.923Z backlog → active (system)

