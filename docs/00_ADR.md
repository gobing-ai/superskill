...
**Detail:** see design-doc-phase2 §4, §9; 04 Phase 2. The data-root rule mirrors the global-vs-local precedence common to dev CLIs (project config shadows home). Proposal path always carries the `<type>/` segment (reconciles the design-doc-phase2 §2.5 path with the G26 acceptance example, which had dropped it).
