...
| D | [Grok Bot opt-in install target (ADR-036/037)](features/D_grok-bot-opt-in-install-target-task-0128-adr-036.md) | F3, B | 🔶 | 0128 and [0130](tasks/0130_prepare-safe-grok-bot-slash-registration-handoffs-within-ins.md) are locally complete; 0129 is cancelled; host visibility remains unverified. [0132](tasks/0132_complete-grok-bot-post-install-skill-registration-with-safe-.md) (P1, D R20–R31) has its primary-fallback registration path implemented and gates green locally; automatic host registration is unavailable by capability disposition, and live host acceptance remains pending. |
