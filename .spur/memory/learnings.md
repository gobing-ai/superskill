...
- 2026-09-14 — A `--worktree` batch in this repo needs the gitignored `vendors/` tree reachable inside the worktree (link it before the first task): without it, `spur-check`'s `skill-citations-resolve` post-check fails on `vendors/rulesync/src/types/hooks.ts` regardless of the task's diff, so the test gate reports an environment gap as a regression.
- 2026-09-14 — The proof-digest bracket hashes the task's `Background`/`Requirements`/`Acceptance Criteria`/`Design`/`Plan` sections, and `spur task record` flips R-item checkboxes inside `Requirements` — so a fresh recomputation after record never equals the recorded digest. A post-record corpus change must re-enter at `test-recheck` (capture-only) → review → verify; re-entering `verify` directly fails its `expect` compare and routes the run to `failed`.
- 2026-09-14 — `verify-answer-lint` keys AC rows by the linked feature's scenario title or the `AC-N` alias, never by prose-form task AC bullets: put task-level AC verdicts in the answer prose and one table row per feature scenario. An AC row marked `MET` on non-executable evidence (e.g. `manual-review`) is force-downgraded to `PARTIAL` and drags the aggregate verdict down, so an AC discharged by a command run must be typed `command` or `test`.
