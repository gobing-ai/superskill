...
/** Recording ProcessExecutor fake — asserts the argv the default installers pass (replaces Bun.spawn stubbing). */
function recordingExecutor(exitFor?: (call: RecordedRun) => number) {
    const calls: RecordedRun[] = [];
    const executor: ProcessExecutor = {
        run: (options: ProcessOptions): Promise<ProcessResult> => {
            const call = { command: options.command, args: options.args ?? [] };
            calls.push(call);
            const exitCode = exitFor?.(call) ?? 0;
...
describe('postInstallOmp', () => {
    ...
    it('does not prune existing modules when hooks are skipped for CLI version compatibility', () => {
        const pluginRoot = makeTempDir('superskill-omp-proot-');
        scaffoldPlugin(pluginRoot, 'cc', false);
        const installPath = makeTempDir('superskill-omp-install-');
        const postDir = join(installPath, 'hooks', 'post');
        mkdirSync(postDir, { recursive: true });
        const existingModule = join(postDir, 'anti-hallucination.js');
        const existingContent =
            '// Generated by superskill install — do not edit manually.\n' +
            '// Command: superskill hook run cc anti-hallucination\n' +
            'export default () => {};\n';
        writeFileSync(existingModule, existingContent);

        const hooksSourceDir = makeTempDir('superskill-omp-hooksrc-');
        writeFileSync(join(hooksSourceDir, 'hooks.json'), JSON.stringify({ hooks: {} }));

        const result = postInstallOmp(pluginRoot, installPath, hooksSourceDir, 'cc', {
            dryRun: false,
            verbose: false,
            skipHooks: true,
        });
        ...
        expect(readFileSync(existingModule, 'utf-8')).toBe(existingContent);
    });
...
describe('defaultRunOmpInstall', () => {
    it('re-registers the marketplace and force-installs so re-installs are idempotent (project scope)', async () => {
        ...
        await defaultRunOmpInstall(
            { source: marketRoot, mode: 'directory' },
            'superskill',
            'demo',
            false,
            fake.executor,
        ...
        expect(argv(fake.calls[0] as RecordedRun)).toEqual(['omp', 'plugin', 'marketplace', 'remove', 'superskill']);
        expect(argv(fake.calls[1] as RecordedRun)).toEqual(['omp', 'plugin', 'marketplace', 'add', marketRoot]);
    ...
    it('passes github owner/repo slug to marketplace add when registration mode is github', async () => {
        ...
        expect(fake.calls).toHaveLength(3);
        expect(argv(fake.calls[0] as RecordedRun)).toEqual(['omp', 'plugin', 'marketplace', 'remove', 'superskill']);
        expect(argv(fake.calls[1] as RecordedRun)).toEqual([
            'omp',
            'plugin',
            'marketplace',
            'add',
        ...
        expect(argv(fake.calls[2] as RecordedRun)).toEqual(['omp', 'plugin', 'install', 'demo@superskill', '--force']);
    });
    ...
    it('tolerates a failing marketplace remove (first install: nothing to remove yet)', async () => {
        ...
        const fake = recordingExecutor((call) => (call.args[1] === 'marketplace' && call.args[2] === 'remove' ? 1 : 0));
    ...
    it('rejects a marketplace name that is not a single path segment, before any spawn', async () => {
        ...
        const marketRoot = makeTempDir('superskill-omp-market-');
        await expect(
            defaultRunOmpInstall({ source: marketRoot, mode: 'directory' }, '..', 'demo', true, fake.executor),
    ...
    it('throws when an omp CLI step exits non-zero instead of reporting success', async () => {
        ...
        const marketRoot = makeTempDir('superskill-omp-market-');
        await expect(
            defaultRunOmpInstall({ source: marketRoot, mode: 'directory' }, 'superskill', 'demo', true, fake.executor),
...
describe('emitHooksForSurrogateTarget', () => {
    it('emits hooks for omp via the pi surrogate source format', () => {
        ...
        const result = emitHooksForSurrogateTarget(
            'omp',
            rulesyncSourceDir,
            outputRoot,
            {
    ...
    it('reconciles policy-excluded hooks through the hook-emit surrogate path', () => {
        const rulesyncSourceDir = makeTempDir('superskill-pi-surrog-');
        writeFileSync(
            join(rulesyncSourceDir, 'hooks.json'),
            JSON.stringify({
                hooks: {
        ...
        const outputRoot = makeTempDir('superskill-pi-out-');
        const piDir = join(outputRoot, '.pi');
        mkdirSync(piDir, { recursive: true });
        writeFileSync(
            join(piDir, 'hooks.json'),
            JSON.stringify({
                hooks: {
                    agent_end: [{ command: 'superskill hook run cc anti-hallucination', timeout: 10 }],
                },
            }),
        );
        ...
        const result = emitHooksForSurrogateTarget(
            ...
            rulesyncSourceDir,
            outputRoot,
            { dryRun: false, global: false },
            'cc',
        );

        expect(result).toMatchObject({ emitted: false, count: 0 });
        expect(JSON.parse(readFileSync(join(piDir, 'hooks.json'), 'utf-8'))).toEqual({ hooks: {} });
    });
});
