...
import ompHookModuleTemplate from './templates/omp-hook-module.txt' with { type: 'text' };
...
function buildModuleContent(hook: ParsedHook): string {
    ...
    // The module source lives in templates/omp-hook-module.txt (an embedded text asset,
    // not inline TS) so the runtime-boundary rules that scan app sources for direct
    // process-spawn specifiers don't trip over generated artifact text. Substitution is
    // single-pass: inserted values are never re-scanned for sentinel tokens.
    const replacements: Record<string, string> = {
        '%%EVENT%%': hook.ompEvent,
        '%%MATCHER_COMMENT%%': hook.matcher === '*' ? 'all tools' : oneLine(hook.matcher),
        '%%COMMAND_COMMENT%%': oneLine(hook.command),
        '%%MATCHER_GUARD%%': matcherGuard,
        '%%COMMAND_LITERAL%%': commandLiteral,
        '%%ARGS_LITERAL%%': argsLiteral,
        '%%TIMEOUT_ARG%%': timeoutArg,
        '%%BLOCK_LOGIC%%': blockLogic,
    };
    return ompHookModuleTemplate.replace(/%%[A-Z_]+%%/g, (token) => replacements[token] ?? token);
}
 ...
 *
 * Owned module reconciliation: before writing new modules, existing generated modules
 * owned by `plugin` (carrying the Superskill banner and a matching command header)
 * are removed from `hooks/pre/` and `hooks/post/`.
 *
 ...
 * @param plugin         - Plugin identifier for ownership reconciliation
 * @param target         - Target name for hook target policy filtering
 * @returns {@link OmpHookResult} with count, file paths, and message.
 */
export function generateOmpHookModules(
    ...
    const BANNER = '// Generated by superskill install — do not edit manually.';

    // Prune existing generated modules owned by `plugin`
    for (const dir of [preDir, postDir]) {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (!content.includes(BANNER)) continue;
                const match = content.match(/\/\/\s*Command:\s*(.+)/);
                if (!match?.[1]) continue;
