...
export function registerSkill(program: Command): void {
    ...
    addStrictOption(
        ...
        await runOperation(async () => {
            const target = resolveTarget(opts);
            const result = await validate('skill', nameOrPath, { target, strict: opts.strict });
    ...
    addSaveOption(
        ...
        await runOperation(async () => {
            const target = resolveTarget(opts);
            const report = await evaluate('skill', nameOrPath, { target, save: opts.save });
    ...
    addSaveOption(
        ...
        await runOperation(async () => {
            const target = resolveTarget(opts);
            await refine('skill', nameOrPath, { target, auto: opts.auto, save: opts.save });
        });
    });
