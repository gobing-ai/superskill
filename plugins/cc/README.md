...
| `validate_response.ts` | Standalone validation CLI (NOT a hook adapter — never wire into hooks.json) — reads response text from `RESPONSE_TEXT` env or stdin, runs `validateResponseText()`, exits 0 (protocol followed) / 1 (violation). On install targets invoke via `superskill script run cc validate-response` (dispatcher deep-imports the engine at CLI build time; works from any cwd). Direct `bun` invocation of the source path is dev-repo-only. Hook enforcement uses `superskill hook run cc anti-hallucination` with exit 2 = block |
