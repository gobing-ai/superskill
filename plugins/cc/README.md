...
| `validate_response.ts` | Standalone validation CLI (NOT a hook adapter — never wire into hooks.json) — reads response text from `RESPONSE_TEXT` env or stdin, runs `validateResponseText()`, exits 0 (protocol followed) / 1 (violation). **Standard install-target form:** stage the entrypoint with `superskill install cc`, then resolve and run via `"$(superskill script path cc anti-hallucination/validate_response.js)"` (portable Node runtime, no Bun on host). **Optional absorbed one-liner:** `superskill script run cc validate-response` (dispatcher deep-imports the engine at CLI build time; works from any cwd; no FS path needed). Direct `bun` invocation of the source `.ts` path is dev-repo-only. Hook enforcement uses `superskill hook run cc anti-hallucination` with exit 2 = block |
