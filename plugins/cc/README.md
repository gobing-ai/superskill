...
| `validate_response.ts` | Standalone validation CLI (NOT a hook adapter — never wire into hooks.json) — reads response text from `RESPONSE_TEXT` env or stdin, runs `validateResponseText()`, exits 0 (protocol followed) / 1 (violation). **Standard install-target form (Entrypoint Contract):** stage + `"$(superskill script path cc anti-hallucination/validate_response.js)"` once a portable `.js` twin is shipped. **Working today / optional absorbed one-liner:** `superskill script run cc validate-response` (dispatcher deep-imports the engine at CLI build time; no FS path needed). Direct `bun` of the source `.ts` path is dev-repo-only. Hook enforcement uses `superskill hook run cc anti-hallucination` with exit 2 = block |
