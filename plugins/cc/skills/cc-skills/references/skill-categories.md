...
- Verification scripts live at the plugin level (`plugins/<plugin>/scripts/<feature>/`), not in the skill folder
...
**Tips**:
- Helper functions live at the plugin level (`plugins/<plugin>/scripts/<feature>/`) for Claude to compose for complex analysis
