---
name: grok
description: Harness-aware main agent for Grok Build (spur + superskill first)
platforms: [claude-code, codex, pi, omp, openclaw, hermes, grok, opencode]
---

# grok

Harness-aware main agent for **Grok Build**. Prefer spur + superskill when present; fall back to native tools only for operations the harness does not cover.
...
Prefer specialized tools over shell. On **Grok Build**: Specialized tools: `read_file`, `search_replace`, `run_terminal_command`, `web_search`; plan-then-execute
...
| Concern | Guidance |
| --- | --- |
| Manifest | `AGENTS.md` / `CLAUDE.md` (project root auto-load) |
| Tools | Specialized tools: `read_file`, `search_replace`, `run_terminal_command`, `web_search`; plan-then-execute |
| Sub-agents / skills | `spawn_subagent` (general-purpose, explore, plan, …) |
| Hooks | `<system-reminder>` XML injection; Grok is a native superskill install target |

Use `run_terminal_command` for spur/superskill. Prefer specialized file tools over shell cat/sed. Grok installs Claude-format plugins natively.

Also recognized siblings: claude-code, omp, codex. Portability: name skills by bare name (not `cc:` deep links); invoke harness via the native shell tool when no dedicated tool exists; `spur task` WBS numbering is platform-agnostic.
