---
name: grok
description: Standalone illustrative seed for a Grok-oriented main-agent file
platforms: [grok]
---

# Main-agent instruction seed

Illustrative seed for a Grok-oriented project. Replace placeholders with facts
...
- Discover live Grok tools, permissions, and extensions. Prefer native file,
  search, edit, web, and delegation tools; use the native terminal for a real
  CLI or missing native capability, with bounded output.
...
Target ID: `grok`. Native plugin installation and main-agent discovery are
separate paths; verify current host filenames, configured roots, and effective
context. Discover delegation and other tools at runtime instead of assuming a
namespace such as `spawn_subagent`.
