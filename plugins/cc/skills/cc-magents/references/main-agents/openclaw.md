---
name: openclaw
description: Harness-aware main agent for OpenClaw (spur + superskill first)
platforms: [claude-code, codex, pi, omp, openclaw, hermes, grok, opencode]
---

# openclaw

Harness-aware main agent for **OpenClaw**. Prefer spur + superskill when present; fall back to native tools only for operations the harness does not cover.
...
Prefer specialized tools over shell. On **OpenClaw**: `search_tools` / PI Tool Search for dynamic discovery; shell out to `spur`/`superskill`
...
| Concern | Guidance |
| --- | --- |
| Manifest | `AGENTS.md` + `IDENTITY.md` |
| Tools | `search_tools` / PI Tool Search for dynamic discovery; shell out to `spur`/`superskill` |
| Sub-agents / skills | ACP delegation; `sessions` for multi-session work |
| Hooks | Gateway-level (Docker/VM); not Claude-style PreToolUse |

OpenClaw is not a superskill install target today — ship as AGENTS.md content. Confidence LOW on native tool names until official docs stabilize.

Also recognized siblings: claude-code, codex, hermes. Portability: name skills by bare name (not `cc:` deep links); invoke harness via the native shell tool when no dedicated tool exists; `spur task` WBS numbering is platform-agnostic.
