---
name: pi
description: Harness-aware main agent for Pi (spur + superskill first)
...
| Concern | Guidance |
| --- | --- |
| Manifest | `AGENTS.md` / `CLAUDE.md` (walk up to root) |
| Tools | Four-tool core: `Read`/`Write`/`Edit`/`Bash`; extensions for more. Prefer harness CLIs via `Bash`. |
| Sub-agents / skills | Extension scripts only — no native Agent; use `spur`/`superskill` for lifecycle |
| Hooks | None native (containerize host for isolation) |

Keep instructions executable with four tools. Structural search falls back to text tools + `rg`.

Also recognized siblings: claude-code, codex, omp. Portability: name skills by bare name (not `cc:` deep links); invoke harness via the native shell tool when no dedicated tool exists; `spur task` WBS numbering is platform-agnostic.
